...
function AddBadge(props: { onAction: () => void }) {
  return (
    <Action.Push
      title="Add Badge"
...
export default function Search() {
  const [badges, setBadges] = useState<BadgerList | null>(null)

  useEffect(() => {
    (async () => setBadges(await storage.getBadges()))()
  }, [])

  const onAction = async (callback?: (badge: Badger) => Promise<void>, badge?: Badger) => {
    if (badge && callback)
